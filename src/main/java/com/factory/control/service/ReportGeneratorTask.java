...
import com.factory.control.service.report.extruder.ExtruderTelemetryReportGenerator;
...
public class ReportGeneratorTask {
    ...
    public void generateReportForPreviousHour() {
        ...
        log.info("Start generating extruder reports for period [%s - %s]", from.toString(), to.toString());
        extruderTelemetryReportGenerator.generateForAllDevicesInPeriod(from, to);
        log.info("Finished generating extruder reports for period [%s - %s]", from.toString(), to.toString());
    }
}
